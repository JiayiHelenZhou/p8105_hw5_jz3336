...
**Comment on Plot 1:** Power is positively related to effect size. Under
the null hypothesis of mu = 0, as the effect size (mu) increased from 1
to 6, the power also increased and reached the highest value 1 when
effect size is 4.
...
**Comment on Plot2:** The estimated average of mu across tests for which
the null was rejected did not equal to the true value of mu until the
true value of mu reached 4. Before mu = 4, the estimated average of mu
across tests for which the null was rejected was greater than the true
value. Starting from mu = 4, the estimated average of rejected mu become
similar to the estimated average of not-rejected mu, which was about the
same as the true mu. This is because the mu in the rejected samples
needs to be different from the true vale of mu to be rejected, while the
estimated mu across all rejected and fail-to-rejected test will have an
average value that is close to the true mean.
